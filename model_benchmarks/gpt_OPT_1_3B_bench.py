...
def prefill_pipelines() -> List[Tuple]:
    ...
    # M is the same as P
    O = [2048]                  # Original input embedding dimension
    D = [2048]                  # Output embedding dimension after projection
    H = [16]                    # Number of heads
    E = [128]                   # Per head embedding dimension
    F = [128]                   # Feature dim per head
    I = [8192]                 # 4× hidden dim, common in FFN

    prod = product(B, P, O, D, H, E, F, I)
    return [ gpt_OPT_1_3B_pipeline(b, p, p, o, d, h, e, f, i, PREFILL_ITERS) for b, p, o, d, h, e, f, i in prod ]
...
def decode_pipelines() -> List[Tuple]:
    ...
    return [ gpt_OPT_1_3B_pipeline(b, p, m, o, d, h, e, f, i, DECODE_ITERS) for b, p, m, o, d, h, e, f, i in prod ]

DECODE_ITERS = [20000, 20000, 20000, 50000, 50000, 50000, 10000, 8000, 8000]
PREFILL_ITERS = [20000, 20000, 20000, 50000, 50000, 50000, 10000, 8000, 8000]

def gpt_OPT_1_3B_pipeline(b, p, m, o, d, h, e, f, i, iters) -> List:
    phases = []
...
pipeline_benchmark(output_dir="benchmarks/gpt_OPT_1_3B_decode", pipelines=decode_pipelines(), device_index=0)
pipeline_benchmark(output_dir="benchmarks/gpt_OPT_1_3B_prefill", pipelines=prefill_pipelines(), device_index=0)
