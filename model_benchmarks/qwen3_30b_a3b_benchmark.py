...
NUM_EXPERTS     = 128
ACTIVE_EXPERTS  = 8
MOE_INTER       = 768   # moe_intermediate_size per expert
...
PREFILL_ITERS = [20000,  20000,  20000,  50000,       50000,  20000,  50000,  20000,  20000]
DECODE_ITERS  = [20000,  20000,  20000,  50000,       50000,  20000,  50000,  20000,  20000]
...
def qwen3_30b_pipeline(b, seq, ctx, iters) -> Tuple[str, List, List]:
    """
    b:   batch size
    ...
    if seq == 1:
        ...
        name = f"qwen3_30b_a3b_prefill_b{b}_s{seq}"
...
def prefill_pipelines() -> List[Tuple]:
    ...
    return [qwen3_30b_pipeline(b, s, s, PREFILL_ITERS) for b, s in product(B, S)]
