...
PREFILL_ITERS = [10000,  10000,  10000,  50000,  50000,  50000,  10000,  500,    500,    500]
DECODE_ITERS  = [10000,  10000,  10000,  50000,  50000,  50000,  10000,  500,    500,    500]


def llama3_1_70b_pipeline(b, seq, ctx, iters) -> Tuple[str, List]:
    """
    b:   batch size
...
pipeline_benchmark(output_dir="benchmarks/llama3.1_70b_prefill", pipelines=prefill_pipelines(), device_index=0)
pipeline_benchmark(output_dir="benchmarks/llama3.1_70b_decode",  pipelines=decode_pipelines(),  device_index=0)