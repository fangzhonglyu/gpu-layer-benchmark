...
def plot_all_energy_subplot(
    ...
    """
    Modified version of plot_all_energy that works with a provided axes object.
    """
    ...
    # colors = ['#ffa600', '#ff6361', '#bc5090', '#4835c3']
    # colors = ["#db9780", "#a94f22", '#8cc5e3', '#3594cc']
    colors = ['tab:blue', 'tab:orange', 'tab:green', 'tab:red']

    colors = {
        "GPU": colors[0],
        "Homogeneous All Nets": colors[1],
        "Homogeneous Per Net": colors[2],
        "Chiplet Pool": colors[3],
    }
    ...
    ax.set_xticks(x + (len(setups)-1)*width/2)
    ax.set_xticklabels(net_names, rotation=45, ha="right", fontsize=6)
    if ylabel:
    ...
    ax.tick_params(axis='y', labelsize=6, pad=2, rotation=0)  

# Method 2: Direct approach using matplotlib subplots
def create_combined_plot_direct(
    savepath: str = "graphs/combined_comparison.png"
):
    """
    Direct approach creating subplots manually.
    """
    fig, axes = plt.subplots(1, 4, figsize=(16, 5))
    fig.subplots_adjust(wspace=-1)
    
    # Plot 1: Energy Comparison
    plot_all_energy_subplot(
        axes[0],
    ...
    plot_all_energy_subplot(
        ...
        ylabel="EDP (Js) × Cost ($)",
        show_legend=True  # Show legend only on last subplot
    )
...
create_combined_plot_direct(savepath='graphs/combined_comparison.png')
