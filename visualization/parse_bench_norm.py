...
def plot_all_energy_subplot_broken(
    ...
    top_margin: float = 22.6, # headroom on the top axis
    ...
    ax_bottom.set_xticklabels(net_names, rotation=35, ha="right", fontsize=9)

    # if ylabel:
    #     ax_bottom.set_ylabel(ylabel, fontsize=9, labelpad=3)
    #     ax_bottom.yaxis.set_label_coords(-0.05, 0.7)
        ...
        for bars_top, vals in top_bar_containers:
            for rect, v in zip(bars_top, vals):
                ...
                ax_top.annotate(
                    ...
                    xy=(cx, 1.2),
                    xytext=(6, 2),         # vertical offset in points
                    textcoords="offset points",
                    ha="center",
                    va="bottom",
                    fontsize=7,
...
def create_combined_plot_direct(savepath: str = "graphs/combined_comparison.png"):
    ...
    # 1) Energy
    ax_top, ax_bottom = make_broken_y_axes_in(axes[0], height_ratios=(1, 3), hspace=0.05)
    ...
    # 2) Energy × Cost
    ax_top, ax_bottom = make_broken_y_axes_in(axes[1], height_ratios=(1, 3), hspace=0.05)
    plot_all_energy_subplot_broken(
        ax_top, ax_bottom,
    ...
    # 3) EDP
    ax_top, ax_bottom = make_broken_y_axes_in(axes[2], height_ratios=(1, 3), hspace=0.05)
    plot_all_energy_subplot_broken(
        ax_top, ax_bottom,
        gpu_energy=gpu_edp,
    ...
    ax_top, ax_bottom = make_broken_y_axes_in(axes[3], height_ratios=(1, 3), hspace=0.05)
    plot_all_energy_subplot_broken(
        ax_top, ax_bottom,
...
# run
create_combined_plot_direct(savepath='graphs/combined_comparison_norm.pdf')
